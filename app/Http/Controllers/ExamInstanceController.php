...
class ExamInstanceController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $exams = Exam_instance::whereNull('is_template')->sortable()->paginate(20);
        $templates = \App\Exam_instance::whereNotNull('is_template')->get();
        $students = Student::all();
        $users = User::all(); //@TODO filter by role.

        return view("examinstance.list")
            ->with('exams', $exams)
            ->with('users', $users)
...
//    }
    ...
    public function detail($id)
    {
        ...
        try {
            return Exam_instance::firstOrFail($id);
...
// then, work out the dependencies from the template. They will have the same order and name, but different ids
            ...
            Userlog::create(['crud' => 'create', 'action' => 'Exam Instance', 'new_value' => $newinstance->name])->save();
            return array(
